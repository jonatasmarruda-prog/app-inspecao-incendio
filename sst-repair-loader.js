...
function loadPhotoMultiFix(){const old=document.getElementById('tbm-photo-multi-fix');if(old)old.remove();const s=document.createElement('script');s.id='tbm-photo-multi-fix';s.src='./photo-multi-fix.js?v=20260903-03&cb='+Date.now();s.async=false;document.body.appendChild(s);return new Promise(resolve=>{s.onload=resolve;s.onerror=resolve})}
...
function loadCloudCrossDevice(){const old=document.getElementById('tbm-cloud-cross-device');if(old)old.remove();const s=document.createElement('script');s.id='tbm-cloud-cross-device';s.src='./cloud-cross-device.js?v=20260903-03&cb='+Date.now();s.async=true;document.body.appendChild(s)}
...
function loadPTAltura(){const old=document.getElementById('tbm-pt-altura');if(old)old.remove();const s=document.createElement('script');s.id='tbm-pt-altura';s.src='./pt-altura.js?v=20260903-02&cb='+Date.now();s.async=false;document.body.appendChild(s);return new Promise(resolve=>{s.onload=resolve;s.onerror=resolve})}
...
function loadMobilePerformanceFix(){const old=document.getElementById('tbm-mobile-performance-fix');if(old)old.remove();const s=document.createElement('script');s.id='tbm-mobile-performance-fix';s.src='./mobile-performance-fix.js?v=20260903-01&cb='+Date.now();s.async=false;document.body.appendChild(s);return new Promise(resolve=>{s.onload=resolve;s.onerror=resolve})}
