...
repairSSTModule();
function loadFireChecklistSplit(){const old=document.getElementById('tbm-fire-checklist-split');if(old)old.remove();const s=document.createElement('script');s.id='tbm-fire-checklist-split';s.src='./fire-checklist-split.js?v=20260903-03&cb='+Date.now();s.async=false;document.body.appendChild(s);return new Promise(resolve=>{s.onload=resolve;s.onerror=resolve})}
...
function loadSaveButtonFix(){const old=document.getElementById('tbm-save-button-fix');if(old)old.remove();const s=document.createElement('script');s.id='tbm-save-button-fix';s.src='./save-button-fix.js?v=20260904-01&cb='+Date.now();s.async=false;document.body.appendChild(s);return new Promise(resolve=>{s.onload=resolve;s.onerror=resolve})}
function loadCloudCrossDevice(){const old=document.getElementById('tbm-cloud-cross-device');if(old)old.remove();const s=document.createElement('script');s.id='tbm-cloud-cross-device';s.src='./cloud-cross-device.js?v=20260903-04&cb='+Date.now();s.async=true;document.body.appendChild(s)}
...
function loadAutoEmailReport(){const old=document.getElementById('tbm-email-relatorio-auto');if(old)old.remove();const s=document.createElement('script');s.id='tbm-email-relatorio-auto';s.src='./email-relatorio-auto.js?v=20260904-06&cb='+Date.now();s.async=false;document.body.appendChild(s);return new Promise(resolve=>{s.onload=resolve;s.onerror=resolve})}
...
function loadMobileInteractionFix(){const old=document.getElementById('tbm-mobile-interaction-fix');if(old)old.remove();const s=document.createElement('script');s.id='tbm-mobile-interaction-fix';s.src='./mobile-interaction-fix.js?v=20260904-01&cb='+Date.now();s.async=false;document.body.appendChild(s);return new Promise(resolve=>{s.onload=resolve;s.onerror=resolve})}
